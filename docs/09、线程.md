...
+ 使用
  ...
  4. 创建需要线程池处理的任务
     ...
     equests = threadpool.makeRequests(go,name_list) 

  5. 将创建的多个任务put到线程池中,threadpool.putRequest

     `for i in Requests: pool.putRequest(i)`

     或：

     `[pool.putRequest(i) for i in requests]`

  6. 等待所有线程 执行完毕 在继续往下 执行   
...
+ threadpool线程池的使用
  ...
  + 方式一
    ...
    pool = threadpool.ThreadPool(5)  # 控制线程的并发数
    start = time.time()
    requests = threadpool.makeRequests(go, name_list)  # 设置参数 转换成线程池对象
    for i in requests: pool.putRequest(i)
    pool.wait()
    ```
  ...
  + 方式二
    ...
    pool.wait()  # 等待所有线程 执行完毕 在继续往下 执行
    print('运行的秒数', time.time() - start)
    ```
